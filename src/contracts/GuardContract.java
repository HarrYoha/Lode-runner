...
public class GuardContract extends GuardDecorator {
    ...
    /*public void checkInvariant(){
        if(getEnvi().getCellNature(getX(),getY()) == Cell.LAD && getY() < getTarget().getY()
            &&getEnvi().getCellNature(getX(),getY() - 1)
        ){

        }
    }*/
    public GuardContract(Guard deleguate) {
        super(deleguate);
    }
    ...
    public boolean climbLeft() {
        int y = getY();
        int x = getX();
        if (getEnvi().getCellNature(delegate.getX(), delegate.getY()) != Cell.HOL) {
            throw new PreconditionException("getCellNature != HOL");
        }
        checkInvariant();
        boolean res = super.climbLeft();
        checkInvariant();
        // \post getX() = 0 implies getX(ClimbLeft()) = getX() and getY(ClimbLeft()) = getY()
        if ((x == 0 && getX() != x && getY() != y)) {
            throw new PostconditionException("ne peut pas grimper si guard a gauche de l'ecran");
        }

        // \post Screen::CellNature(getX()-1,getY() +1) in {MTL, PLT }
        // implies getX(ClimbLeft()) = getX() and getY(ClimbLeft()) = getY()
        if (!getEnvi().getCellNature(x - 1, y + 1).isFree()) {
            if (getX() != x && getY() != y)
                throw new PostconditionException("ne peut pas grimper si la case en haut a gauche n'est pas libre");
        }


        //\post exists Character c in Environment::CellContent(getX()-1,getY()+1)
        //implies getX(ClimbLeft()) = getX() and getY(ClimbLeft()) = getY()
        if (!containsNoneOrTreasure(x - 1, y + 1)) {
            if (getX() != x && getY() != y)
                throw new PostconditionException("ne peut pas grimper si la case en haut a gauche n'est pas dispo");
        }
        ...
        if (x != 0 && getEnvi().getCellNature(x - 1, y + 1).isFree() && containsNoneOrTreasure(x - 1, y + 1)
        && !getEnvi().getCellNature(x - 1, y + 1).isFree() ||getEnvi().getCellNature(x - 1, y + 1) == Cell.LAD) {
            if (getX() != x - 1 && getY() != y + 1)
                throw new PostconditionException("le guard ne s'est pas deplacé");
        }
        return res;
    }
    ...
    public boolean climbRight() {
        ...
        checkInvariant();
        boolean res = super.climbRight();

        //\post getX() = Screen::getWidth() implies getX(ClimbRight()) = getX() and getY(ClimbRight()) = getY()
        if ((x == getEnvi().getWidth() && getX() != x && getY() != y)) {
            throw new PostconditionException("ne peut pas grimper si guard a droite de l'ecran");
        }

        // \post Screen::CellNature(getX()+1,getY() +1) in {MTL, PLT }
        // implies getX(ClimbRight()) = getX() and getY(ClimbRight()) = getY()
        if (!getEnvi().getCellNature(x + 1, y + 1).isFree()) {
            if (getX() != x && getY() != y)
                throw new PostconditionException("ne peut pas grimper si la case en haut a droite n'est pas libre");
        }
        ...
        if (!containsNoneOrTreasure(x + 1, y + 1)) {
            if (getX() != x && getY() != y)
                throw new PostconditionException("ne peut pas grimper si la case en haut a droite n'est pas dispo");
        }


        //\post getX() = Screen::getWidth() and Screen::CellNature(getX()+1,getY()+1) not in {MTL, PLT }
        // and not exists Character c in Environment::CellContent(getX()+1,getY()+1)
        // implies getX(ClimbLeft()) = getX()+1 and getY(ClimbLeft()) = getY()+1
        if (x != getEnvi().getWidth() && getEnvi().getCellNature(x + 1, y + 1).isFree() &&
                containsNoneOrTreasure(x + 1, y + 1) && !getEnvi().getCellNature(x + 1, y + 1).isFree()
                ||getEnvi().getCellNature(x + 1, y + 1) == Cell.LAD) {
            if (getX() != x + 1 && getY() != y + 1)
                throw new PostconditionException("le guard ne s'est pas deplacé");
        }
        return res;
    ...
    //on peut pas faire de double heritage donc CC de character
